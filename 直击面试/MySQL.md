...
每提交一个事务必须先将该事务的所有日志写入到重做日志文件进行持久化，数据库就可以通过重做日志来保证事务的持久性。

每当有修改事务时，还会产生 undo log，如果需要回滚，则根据 undo log 的反向语句进行逻辑操作，比如 insert 一条记录就 delete 一条记录。所以undo log 主要实现了数据库的原子性。
...
![image-20210822181340692](../_img/mysql-2.png)
...
![image-20210822181416382](../_img/mysql-3.png)

详细解读参考：[MySQL三大日志(binlog、redo log和undo log)详解](https://javaguide.cn/database/mysql/mysql-logs.html)
...
![image-20210822204026552](../_img/mysql-13.png)
