...
![并行和并发](https://img-blog.csdnimg.cn/img_convert/6250b4d5f38b01f9cb737982844cb0d5.png)

就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。

![并行并发和食堂打饭](https://img-blog.csdnimg.cn/img_convert/7523f374a2fc3a6e6a76d636ef2e327a.png)
...
![程序进程线程关系](https://img-blog.csdnimg.cn/img_convert/1d6734ea65e14caa4b28e255c01f786b.png)
...
![线程创建三种方式](https://img-blog.csdnimg.cn/img_convert/547668337faf020b8c1d685d4e72138b.png)
...
![start方法](https://img-blog.csdnimg.cn/img_convert/0908a05d48ed30922b96f4acd585aacd.png)

**为什么我们不能直接调用run()方法？**也很清楚， 如果直接调用Thread的run()方法，那么run方法还是运行在主线程中，相当于顺序执行，就起不到多线程的效果。

## 5.线程有哪些常用的调度方法？

![线程常用调度方法](https://img-blog.csdnimg.cn/img_convert/bad3ec0ff77808bce0f6bbc3a7090647.png)
...
![Java线程状态变化](https://img-blog.csdnimg.cn/img_convert/56d4671b23f44cf1596fe6b73da69856.png)

## 7.什么是线程上下文切换？

使用多线程的目的是为了充分利用CPU，但是我们知道，并发其实是一个CPU来应付多个线程。

![线程切换-2020-12-16-2107](https://img-blog.csdnimg.cn/img_convert/4af558e7d833dfc018b25c40167751e0.png)

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![上下文切换时机](https://img-blog.csdnimg.cn/img_convert/e5b1a5cd4b179fe01a5ac12cfb6dab92.png)
...
![线程间通信方式](https://img-blog.csdnimg.cn/img_convert/ebeb425fbc43c3826b518ec2d815b821.png)
...
![ThreadLocal线程副本](https://img-blog.csdnimg.cn/img_convert/5e5bd9b42a3a92565f2289a01731cece.png)
...
![ThreadLoca存放用户上下文](https://img-blog.csdnimg.cn/img_convert/6fd85251639485b60a7d7398cb17cb33.png)
...
![ThreadLoca结构图](https://img-blog.csdnimg.cn/img_convert/800c85b4dd8c6753586e019b017d7539.png)
...
![ThreadLocal内存分配](https://img-blog.csdnimg.cn/img_convert/6a5cad137163696106a0f18d5ed438e3.png)
...
![ThreadLocalMap结构示意图](https://img-blog.csdnimg.cn/img_convert/8cb25f2a3ddae79b7e9312e74d35284f.png)
...
![ThreadLocalMap解决冲突](https://img-blog.csdnimg.cn/img_convert/f854fa40555eeaa1c1bdae2e56c4182c.png)
...
![ThreadLocalMap扩容](https://img-blog.csdnimg.cn/img_convert/6b197de56b8d59d7f65d351e1ce9322c.png)

具体代码：

![ThreadLocalMap resize](https://img-blog.csdnimg.cn/img_convert/4732c9d88e3fde033b056771d8ff2409.png)
...
![Java内存模型](https://img-blog.csdnimg.cn/img_convert/b984e401f5814ede9f02955c09946fc4.png)

本地内存是JMM的 一个抽象概念，并不真实存在。它其实涵盖了缓存、写缓冲区、寄存器以及其他的硬件和编译器优化。

![实际线程工作模型](https://img-blog.csdnimg.cn/img_convert/a405e3802f26fde24547856f6e8078c1.png)
...
![多级指令重排](https://img-blog.csdnimg.cn/img_convert/bc91d28526e3524b422fdd9bc5e8ebe2.png)
...
![双重校验单例模式异常情形](https://img-blog.csdnimg.cn/img_convert/a83cb74f88c54b34a2833ad83148efc6.png)
...
![happens-before六大规则](https://img-blog.csdnimg.cn/img_convert/29be8df44f6bc3f4f2bfe40988030d6c.png)
...
![image-20210812200646364](https://img-blog.csdnimg.cn/img_convert/e86b8a7966e0209e19cd1aaf923d66be.png)

A和C之间存在数据依赖关系，同时B和C之间也存在数据依赖关系。因此在最终执行的指令序列中，C不能被重排序到A和B的前面（C排到A和B的前面，程序的结果将会被改变）。但A和B之间没有数据依赖关系，编译器和处理器可以重排序A和B之间的执行顺序。

所以最终，程序可能会有两种执行顺序：

![两种执行结果](https://img-blog.csdnimg.cn/img_convert/6f343b027946c15f4419c8086fdbb4f3.png)
...
![volatile内存可见性](https://img-blog.csdnimg.cn/img_convert/0314bbe06d1d54e01798503ed3760621.png)

> volatile怎么保证有序性的呢？

重排序可以分为编译器重排序和处理器重排序，valatile保证有序性，就是通过分别限制这两种类型的重排序。

![volatile重排序规则表](https://img-blog.csdnimg.cn/img_convert/1d6b683b0b9e9df499aaf9d69aa7c29e.png)
...
![volatile写插入内存屏障后生成的指令序列示意图](https://img-blog.csdnimg.cn/img_convert/f0c8bf9fa41e7fa9ce873d7ee8c5ad5e.png)

![volatile写插入内存屏障后生成的指令序列示意图](https://img-blog.csdnimg.cn/img_convert/eb72619604b0fe38d96f7e9227cb6771.png)
...
![monitorenter和monitorexit](https://img-blog.csdnimg.cn/img_convert/e07cc22241f955da121c67e1038820e7.png)

1. synchronized修饰同步方法时，JVM采用`ACC_SYNCHRONIZED`标记符来实现同步，这个标识指明了该方法是一个同步方法。

   同样可以写段代码反编译看一下。

![synchronized修饰同步方法](https://img-blog.csdnimg.cn/img_convert/47247703dcf50b93d712bccb994e7832.png)
...
![就诊-图片来源参考[18]](https://img-blog.csdnimg.cn/img_convert/bc0aefb2b1724fe550bccdec785e570d.png)
...
![Java Montior机制](https://img-blog.csdnimg.cn/img_convert/a83b81aafe055a1e8883c35a8e94c163.png)
...
![Mark Word变化](https://img-blog.csdnimg.cn/img_convert/a46e103f3d517db942ddb67c831d96aa.png)
...
![锁升级方向](https://img-blog.csdnimg.cn/img_convert/dc6ff772f99fbcf03fdc7962be33b92d.png)
...
![锁升级简略过程](https://img-blog.csdnimg.cn/img_convert/ca14d1903ded65655cd8c911da6b676e.png)

完整的升级过程：

![synchronized 锁升级过程-来源参考[14]](https://img-blog.csdnimg.cn/img_convert/84eb7c1a48869f932d4398c60e77f176.png)
...
![synchronized和ReentrantLock的区别](https://img-blog.csdnimg.cn/img_convert/bff18d56d1d05f1c18e671e82a5ea83b.png)
...
![AQS抽象队列同步器](https://img-blog.csdnimg.cn/img_convert/bfedaad33a150ff42b2e348795c32153.png)先简单了解一下CLH：Craig、Landin and Hagersten 队列，是 **单向链表实现的队列**。申请线程只在本地变量上自旋，**它不断轮询前驱的状态**，如果发现 **前驱节点释放了锁就结束自旋**

![CLH队列](https://img-blog.csdnimg.cn/img_convert/4347e31e9239873479671c27b86d558b.png)

AQS 中的队列是 CLH 变体的虚拟双向队列，通过将每条请求共享资源的线程封装成一个节点来实现锁的分配：

![AQS变种CLH队列](https://img-blog.csdnimg.cn/img_convert/6ea73bb03eafd9bbdce38a4c83952a90.png)
...
![ReentrantLock 非公平锁加锁流程简图](https://img-blog.csdnimg.cn/img_convert/a193d7522d10bcc49e3a8cb1e61e8936.png)
...
![公平锁tryAcquire](https://img-blog.csdnimg.cn/img_convert/591c4ca996afd8019733394ecca8c54f.png)
...
![CAS三大问题](https://img-blog.csdnimg.cn/img_convert/f828e595ccc51bb7166dd5ad2cd10a59.png)
...
![Java保证原子性方法](https://img-blog.csdnimg.cn/img_convert/b19ac1ff0c7a62d5c35ad289e03a7938.png)
...
![原子操作类](https://img-blog.csdnimg.cn/img_convert/3ca137f7af1fa2b98c181ac65d499f3f.png)
...
![死锁示意图](https://img-blog.csdnimg.cn/img_convert/81ab53f8a023aca89f776d9914eb86aa.png)

那么为什么会产生死锁呢？ 死锁的产生必须具备以下四个条件：

![死锁产生必备四条件](https://img-blog.csdnimg.cn/img_convert/bcd8b1c5f4519a42601ff3351d446bb6.png)
...
![线程死锁检测](https://img-blog.csdnimg.cn/img_convert/b969fe45e242a846eb53bdf9b4281e88.png)
...
![王者荣耀等待玩家确认-来源参考[18]](https://img-blog.csdnimg.cn/img_convert/a2d68def43fd6e27cbc0d4f76a79a805.png)
...
![王者荣耀-来源参考[18]](https://img-blog.csdnimg.cn/img_convert/ec484cedcb22cd82357fa95287fdbd15.png)
...
![想结衣结衣不依,迷爱理爱理不理。](https://img-blog.csdnimg.cn/img_convert/e969d5fa47b5aaeaf295ee93455ede56.png)

我们拿代码模拟这一场景，发现CountDownLatch无能为力了，因为CountDownLatch的使用是一次性的，无法重复利用，而这里等待了两次。此时，我们用CyclicBarrier就可以实现，因为它可以重复利用。

![小约翰可汗选妃模拟代码](https://img-blog.csdnimg.cn/img_convert/272ff11dde9bca56933823aa1c42418f.png)

运行结果：

![运行结果](https://img-blog.csdnimg.cn/img_convert/d0101ba6fa2337ce1d855018e677c368.png)
...
![CyclicBarrier工作流程](https://img-blog.csdnimg.cn/img_convert/8a255b8d54dbe6d16431a80540bb9505.png)
...
![停车场空闲车位提示-图片来源网络](https://img-blog.csdnimg.cn/img_convert/696a700d36410d48439ffad99ef15f14.png)

我们把这个例子类比一下，车辆就是线程，进入停车场就是线程在执行，离开停车场就是线程执行完毕，看见红灯就表示线程被阻塞，不能执行，Semaphore的本质就是**协调多个线程对共享资源的获取**。

![Semaphore许可获取-来源参考[18]](https://img-blog.csdnimg.cn/img_convert/9b13827e9c8fb2c3efdb13705deec314.png)
...
![英雄交换猎物-来源参考[18]](https://img-blog.csdnimg.cn/img_convert/f396806d7740c2bb8b57921705ae6265.png)
...
![管理线程的池子](https://img-blog.csdnimg.cn/img_convert/eccc984daf63f5158442fba22298446a.png)
...
![业务示例](https://img-blog.csdnimg.cn/img_convert/c8f9d3e55d58dbf330f336f6fb9a02c6.png)
...
![直接办理](https://img-blog.csdnimg.cn/img_convert/7fdfc4ff5bf3b3e688307de8895ee1af.png)

1. 老三发现没有空闲的窗口，就在排队区排队等。

![排队等待](https://img-blog.csdnimg.cn/img_convert/b52ec12d42ade76af65f6917550ae393.png)

1. 老三发现没有空闲的窗口，等待区也满了，蚌埠住了，经理一看，就让休息的小姐姐赶紧回来上班，等待区号靠前的赶紧去新窗口办，老三去排队区排队。小姐姐比较辛苦，假如一段时间发现他们可以不用接着营业，经理就让她们接着休息。

![排队区满](https://img-blog.csdnimg.cn/img_convert/45acd5e84935162228e67bacff174000.png)

1. 老三一看，六个窗口都满了，等待区也没位置了。老三急了，要闹，经理赶紧出来了，经理该怎么办呢？

![等待区，排队区都满](https://img-blog.csdnimg.cn/img_convert/193a30249a2a6b52d5f5021484319d0f.png)
...
![线程池执行流程](https://img-blog.csdnimg.cn/img_convert/8c78283eeff1a4e9982e5b126a651839.png)
...
![线程池参数](https://img-blog.csdnimg.cn/img_convert/ede64e0812d3c9879fd52e96b06bb9bd.png)
...
![四种策略](https://img-blog.csdnimg.cn/img_convert/ad4a1ea048caa269b1e2cbed9d2b47f6.png)
...
![线程池常用阻塞队列](https://img-blog.csdnimg.cn/img_convert/1e4623860624213eaa1ac950257a7e22.png)
...
![常见线程池参数配置方案-来源美团技术博客](https://img-blog.csdnimg.cn/img_convert/32c50db7191aea232745bc6e229a34d7.png)
...
![四大线程池](https://img-blog.csdnimg.cn/img_convert/dfe1b7ba309f2ee3bb6fc62baf5df6c9.png)
...
![SingleThreadExecutor运行流程](https://img-blog.csdnimg.cn/img_convert/5c5bbd78dcf6c6372c1023f084e31030.png)
...
![FixedThreadPool](https://img-blog.csdnimg.cn/img_convert/1162b40b2772e0d382f6d2160776feca.png)
...
![CachedThreadPool执行流程](https://img-blog.csdnimg.cn/img_convert/2a88dd606243a39750154f4cfad8215b.png)
...
![ScheduledThreadPool执行流程](https://img-blog.csdnimg.cn/img_convert/eb41b17b18f4538095a24865dcd59679.png)
...
![ScheduledThreadPoolExecutor执行流程](https://img-blog.csdnimg.cn/img_convert/e7651c72b0394c7482286928f5db9a69.png)
...
![线程池异常处理](https://img-blog.csdnimg.cn/img_convert/cb8469e260d3d5dd23cd06793361b590.png)
...
![线程池状态切换图](https://img-blog.csdnimg.cn/img_convert/98a8f9b120c4a1db4d25f943148e65d4.png)
...
![JDK 线程池参数设置接口来源参考[7]](https://img-blog.csdnimg.cn/img_convert/e06a30d945a861cc574eb8720ccaaded.png)

这里主要有两个思路：

![动态修改线程池参数](https://img-blog.csdnimg.cn/img_convert/bed77cde2cb5a512481159a22feb8afc.png)
...
![线程池评估方案 来源参考[7]](https://img-blog.csdnimg.cn/img_convert/ea3765d91e8473c043a1242256f01e25.png)
...
![线程池调优](https://img-blog.csdnimg.cn/img_convert/895612877cb4352edd8da7c1828e90c4.png)
...
![线程池主要实现流程](https://img-blog.csdnimg.cn/img_convert/b3750254b7b7f0575f7ced605d204235.png)
...
![自定义线程池](https://img-blog.csdnimg.cn/img_convert/3179b40bddd102da4ae9551c58d45d69.png)
...
![Fork/Join分治算法](https://img-blog.csdnimg.cn/img_convert/96c38e49e64511788caee0a6c9e5c001.png)
...
![工作窃取](https://img-blog.csdnimg.cn/img_convert/037201dfbc3f3e132b4645d964781d82.png)
