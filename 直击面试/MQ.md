...
> 以RocketMQ为例，转载自[面渣逆袭：RocketMQ二十三问](https://mp.weixin.qq.com/s?__biz=MzkwODE5ODM0Ng==&mid=2247490996&idx=1&sn=ba9558574d71979aa689a710c28c7e0e&scene=21#wechat_redirect)
...
![RocketMQ架构](../_img/RocketMQ架构.png)
...
![RocketMQ类比邮政体系](../_img/RocketMQ类比邮政体系.png)
...
![RocketMQ存储](../_img/RocketMQ存储.png)
...
![同步刷盘和异步刷盘-图片来源官网](../_img/同步刷盘和异步刷盘.png)
...
![延迟消息处理流程-图片来源见水印](../_img/延迟消息处理流程.png)
...
![RocketMQ实现消息事务](../_img/RocketMQ实现消息事务.png)
...
![NameServer集群](../_img/NameServer集群.png)

RocketMQ的高可用主要是在体现在Broker的读和写的高可用，Broker的高可用是通过`集群`和`主从`实现的。

![Broker集群、主从示意图](../_img/Broker集群、主从示意图.png)
...
![RocketMQ整体工作流程](../_img/RocketMQ整体工作流程.png)
...
RocketMQ主要的存储文件包括CommitLog文件、ConsumeQueue文件、Indexfile文件。![RocketMQ主要存储文件](../_img/RocketMQ主要存储文件.png)
...
- **ConsumeQueue**：消息消费队列，引入的目的主要是提高消息消费的性能，由于RocketMQ是基于主题topic的订阅模式，消息消费是针对主题进行的，如果要遍历commitlog文件中根据topic检索消息是非常低效的。
  ...
  ![Comsumer Queue](../_img/Comsumer_Queue.png)
...
- **IndexFile**：IndexFile（索引文件）提供了一种可以通过key或时间区间来查询消息的方法。Index文件的存储位置是：{fileName}，文件名fileName是以创建时的时间戳命名的，固定的单个IndexFile文件大小约为400M，一个IndexFile可以保存 2000W个索引，IndexFile的底层存储设计为在文件系统中实现HashMap结构，故RocketMQ的索引文件其底层实现为hash索引。
  ...
  ![IndexFile文件示意图-来源参考[2]](../_img/IndexFile文件示意图.png)
...
![RocketMQ存储总结](../_img/RocketMQ存储总结.png)
...
![传统文件传输示意图-来源《图解操作系统》](../_img/传统文件传输示意图.png)
...
![mmap示意图-来源《图解操作系统》](../_img/mmap示意图.png)
...
![Producer的负载均衡](../_img/Producer的负载均衡.png)
...
![RebalanceImpl](../_img/RebalanceImpl.png)
...
![Cosumer分配](../_img/Cosumer分配.png)

(4) 然后，调用updateProcessQueueTableInRebalance()方法，具体的做法是，先将分配到的消息队列集合（mqSet）与processQueueTable做一个过滤比对。![updateProcessQueueTableInRebalance](../_img/updateProcessQueueTableInRebalance.png)
